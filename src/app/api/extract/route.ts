...
7. line_items: เก็บทุกบรรทัดสินค้า/บริการ ตาม description ที่พิมพ์จริง ห้ามรวมบรรทัดส่วนลด/ยอดรวมเข้าเป็นสินค้า amount ของแต่ละบรรทัดใช้ตามคอลัมน์จำนวนเงินที่พิมพ์ (ซึ่งมักเป็นยอดสุทธิหลังส่วนลดของบรรทัดแล้ว เช่นกรณีส่วนลดซ้อน "30+25%") ห้ามคำนวณเองจาก ราคา×จำนวน ถ้าเป็นบรรทัด**ของแถม** (ราคา 0.00 หรือมีคำว่า แถม/ของแถม/ฟรี) ให้เก็บเป็นบรรทัดแยกตามที่พิมพ์ ใส่ quantity ตามจริง unit_price = 0 amount = 0 และคงคำว่า "(แถม)" ไว้ในชื่อ ห้ามข้ามบรรทัดนี้ เพราะระบบจะเอาจำนวนแถมไปหารต้นทุนต่อหน่วยให้
...
9. ตัวเลขเงิน: ตอบเป็น number ไม่มี comma. subtotal = ฐานก่อน VAT **หลังหักส่วนลดท้ายบิลแล้ว** (คือช่อง "รวมราคาสินค้า/มูลค่าสินค้า" ที่ใช้คิด VAT = total − vat_amount). discount = ผลรวมส่วนลดท้ายบิล**เป็นบาท**: ถ้าลดซ้อนหลายชั้น (เช่น ลด 25% ได้ 33,990 แล้วลดอีก 5% ได้ 5,098.50) ให้บวกยอดบาททุกชั้นเข้าด้วยกัน (= 39,088.50) ห้ามเอา % มาบวกกันแล้วคูณครั้งเดียว. vat_amount = จำนวน VAT ตามที่พิมพ์, total = ยอดสุทธิที่ต้องชำระจริง (รวมเงินทั้งสิ้น)
10. ก่อนตอบ ตรวจทานให้ยอดกระทบกันครบวง: ผลรวมรายการ − discount ≈ subtotal และ subtotal + vat_amount ≈ total (ถ้าราคารวม VAT แล้ว vat_amount จะถูกถอดออกจาก total อยู่แล้ว จึงได้ subtotal + vat_amount ≈ total เช่นกัน) ถ้ายังไม่ตรงให้อ่านตัวเลขซ้ำ และถ้ายังไม่ตรงอีกให้ระบุใน warnings
